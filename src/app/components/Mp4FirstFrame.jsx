...
export default function Mp4FirstFrame({ config: controlledConfig, onConfigChange }) {
  ...
  const handleConvert = async () => {
    ...
    setConverting(true);
    const toastId = message.loading('开始提取首帧...', 0);
    ...
    try {
      ...
      for (let i = 0; i < filesToConvert.length; i++) {
        ...
        const response = await fetch('/api/mp4-first-frame', {
          method: 'POST',
          body: formData,
        });
      ...
      // 设置自动关闭时间
      message.success('所有文件首帧提取完成', 3);
    } catch (error) {
      console.error('提取首帧失败:', error);
      message.error(`提取首帧失败: ${error.message}`);
    } finally {
      setConverting(false);
      if (toastLoading) {
  ...
  const handleDownload = (file) => {
    ...
    link.download = `${file.file.name.replace(/\.[^/.]+$/, '')}_first_frame.${config.format}`;
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      for (const file of successFiles) {
        ...
        const blob = await response.blob();
        const fileName = `${file.file.name.replace(/\.[^/.]+$/, '')}_first_frame.${config.format}`;
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `mp4-first-frames.${config.format === 'webp' ? 'webp' : 'png'}.zip`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card title="上传 MP4 文件">
        <input
          type="file"
          id="file-upload-mp4-first-frame"
          accept="video/mp4"
          multiple
          style={{ display: 'none' }}
          onChange={(e) => {
                  ...
                  video.onloadedmetadata = () => {
                    ...
                    canvas.toBlob((blob) => {
                      ...
                      setUploadedFiles(current => current.map(f => 
                        f.uid === newFile.uid ? { ...f, previewUrl } : f
                      ));
                    }, 'image/jpeg', 0.8);
                  };
        ...
        <Button
          ...
          size="large"
          type="primary"
          onClick={() => document.getElementById('file-upload-mp4-first-frame').click()}
        >
          选择多个 MP4 文件
        </Button>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                    ...
                    >
                      ...
                      {item.status === 'converting' && '提取中'}
                      {item.status === 'success' && '提取成功'}
                      {item.status === 'failed' && '提取失败'}
                    </Tag>
                  </div>
                  <Space orientation="vertical" style={{ width: '100%' }}>
                    ...
                    {item.status === 'failed' && (
                      <Text type="danger">{item.error || '提取失败'}</Text>
                    )}
      ...
      <Card title="提取配置">
        <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
          <div>
            ...
            <Text type="secondary">选择提取首帧的输出格式</Text>
          </div>

          <div>
            <Text strong>图像质量 ({config.quality})</Text>
            <Slider
              min={1}
              max={100}
            ...
            />
            <Text type="secondary">控制输出图像的质量</Text>
          </div>

          <div>
            <Text strong>处理效率 ({config.effort})</Text>
            <Slider
              min={1}
              max={6}
              value={config.effort}
            ...
            />
            <Text type="secondary">值越高，处理越慢，但输出文件可能更小（仅WebP格式有效）</Text>
          </div>
        </Space>
      </Card>
