...
export default function GifToWebp({ config: controlledConfig, onConfigChange }) {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      for (let i = 0; i < filesToConvert.length; i++) {
        ...
        // 更新当前文件状态为成功
        setUploadedFiles(prev => prev.map(f => 
          f.uid === file.uid ? { ...f, status: 'success', progress: 100, convertedUrl } : f
        ));
      }
  ...
  const handleDownload = (file) => {
    ...
    link.download = `${file.file.name.replace('.gif', '.webp')}`;
    ...
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Card title="上传 GIF 文件">
        ...
        <Button
          ...
          size="large"
          type="primary"
          onClick={() => document.getElementById('file-upload-gif').click()}
        >
          选择多个 GIF 文件
        </Button>
